...
export async function getMetadataByTicker(
  ticker: string,
...
export async function getMetadataStats() {
  ...
  const totalTokens = allMetadata.reduce(
    (sum, m) => sum + m.usage.total_tokens,
    0,
  );
  ...
  const byType = allMetadata.reduce(
    (acc, m) => {
  ...
  const byTicker = allMetadata.reduce(
    (acc, m) => {
      if (!acc[m.ticker]) {
  ...
  const averageCost =
    allMetadata.length > 0 ? totalCost / allMetadata.length : 0;
  const averageDuration =
    allMetadata.length > 0 ? totalDuration / allMetadata.length : 0;
  ...
  return {
    ...
    byType,
    byTicker,
    mostExpensiveTicker: Object.entries(byTicker).sort(
      (a, b) => b[1].totalCost - a[1].totalCost,
    )[0],
    mostResearchedTicker: Object.entries(byTicker).sort(
...
export async function viewMetadata() {
  ...
  if (stats.mostResearchedTicker) {
    console.log(
      `📈 Most Researched Ticker: ${stats.mostResearchedTicker[0]} (${stats.mostResearchedTicker[1].count} runs)`,
    );
  }
...
if (typeof window !== "undefined") {
  ...
  console.log(
    "💡 Research metadata utilities loaded. Use window.researchMetadata to access:",
  );
