...
export class AgentService {
                ...
                switch (event.type) {
                    ...
                    case 'item.completed':
                        ...
                        if (event.item.type === 'todo_list') {
                            const todos = event.item.items.map((t, i) => `${i + 1}. [${t.completed ? '✓' : ' '}] ${t.text}`).join('\n     ');
                            console.log(`  📋 Plan:\n     ${todos}`);
                        }
                        else if (event.item.type === 'reasoning') {
    ...
    async reevaluate(request, existingReport) {
        ...
        try {
            ...
            throw new Error(`Reevaluation failed: ${error.message}`);
        }
    }
    async chat(ticker, message, reportContent) {
